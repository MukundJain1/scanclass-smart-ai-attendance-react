...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ faceImage, onSuccess, onCancel }) => {
  ...
  const startRecording = async () => {
    ...
    } catch (error) { setDialog({ isOpen: true, title: 'Error', message: 'Microphone access denied.', type: 'error', onConfirm: () => setDialog((prev: DialogProps) => ({...prev, isOpen: false}))}); }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDialog({ isOpen: true, title: 'Registration Failed', message: error.response?.data?.detail || "Network error. Please try again.", type: 'error', onConfirm: () => setDialog((prev: DialogProps) => ({...prev, isOpen: false})) });
    } finally {
