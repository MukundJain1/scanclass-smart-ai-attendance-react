...
const StudentDashboard: React.FC<DashboardProps> = ({ studentData, showToast }) => {
  ...
  const showDialog = (title: string, message: string, type: DialogProps['type'] = 'alert', confirmAction?: () => void) => {
    setDialog({ isOpen: true, title, message, type, onConfirm: () => { setDialog(prev => ({ ...prev, isOpen: false })); if(confirmAction) confirmAction(); }, onCancel: type === 'confirm' ? () => setDialog(prev => ({ ...prev, isOpen: false })) : undefined });
  };
